...
import com.mangalhousemanager.databinding.ActivityMainBinding;
import com.mangalhousemanager.fragment.DashboardFragment;
import com.mangalhousemanager.fragment.FragmentScanner;
import com.mangalhousemanager.fragment.MenuFragment;
import com.mangalhousemanager.fragment.NewDashboardFragment;
import com.mangalhousemanager.fragment.OrderHistoryFragment;
import com.mangalhousemanager.fragment.ReservationFragment;
...
public class MainActivity extends AppCompatActivity {
    ...
    public static LinearLayout linearLayoutOrder, llorderHistory;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        linearLayoutOrder = binding.llOrder;
        llorderHistory = binding.llOrderHistory;
        binding.address.setText(storeUserData.getString(Constants.street) + ", " + storeUserData.getString(Constants.city) + ", " + storeUserData.getString(Constants.country));
        binding.resEmail.setText(storeUserData.getString(Constants.email));
        binding.resName.setText(storeUserData.getString(Constants.name));

        linearLayoutOrder.setOnClickListener( new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                changeFragment(new DashboardFragment(), Constants.DASHBOARD);
            }
        } );

        //changeFragment(dashboardFragment, Constants.DASHBOARD);
        changeFragment(newDashboardFragment, Constants.DASHBOARD);

        if (storeUserData.getInt(Constants.res_status) == 1) {
            binding.switchStatus.setImageResource(R.drawable.switchon);
        } else {
            binding.switchStatus.setImageResource(R.drawable.switchoff);
        }

        binding.switchStatus.setOnClickListener(view -> {

            if (storeUserData.getInt(Constants.res_status) == 1) {
                new AlertDialog.Builder(activity)
                        .setMessage(Constants.CONFIRMATION_ORDER_DISABLE)
                        .setPositiveButton(Constants.YES_LABEL, (dialog, which) -> {
                            changeRestaurantStatus(0);
                        })
        ...
        binding.llSetting.setOnClickListener(view -> {
            binding.switchStatus.setVisibility(View.INVISIBLE);
            binding.drawerLayout.closeDrawer(Gravity.START);
            binding.imgFilter.setVisibility(View.INVISIBLE);
            changeFragment(new SettingFragmenty(), Constants.SETTING);
        });

        binding.llReservation.setOnClickListener(view -> {
            binding.switchStatus.setVisibility(View.INVISIBLE);
            binding.drawerLayout.closeDrawer(Gravity.START);
            binding.imgFilter.setVisibility(View.INVISIBLE);
            changeFragment(new ReservationFragment(), Constants.SETTING);
        });

        binding.llScanBarcode.setOnClickListener(view -> {
            binding.switchStatus.setVisibility(View.INVISIBLE);
            binding.drawerLayout.closeDrawer(Gravity.START);
            binding.imgFilter.setVisibility(View.INVISIBLE);
            changeFragment(new FragmentScanner(), Constants.SCANNER);
        });
        ...
        binding.llOrderHistory.setOnClickListener(view -> {
            ...
            changeFragment(new OrderHistoryFragment(), Constants.PAST_ORDERS);
        });

        binding.llHome.setOnClickListener(view -> {
            binding.switchStatus.setVisibility(View.VISIBLE);
            binding.drawerLayout.closeDrawer(Gravity.START);
            binding.imgFilter.setVisibility(View.INVISIBLE);
            changeFragment(newDashboardFragment, Constants.DASHBOARD);
        });

        binding.llReview.setOnClickListener(view -> {
            binding.switchStatus.setVisibility(View.GONE);
            binding.drawerLayout.closeDrawer(Gravity.START);
            binding.imgFilter.setVisibility(View.INVISIBLE);
            changeFragment(new ReviewFragment(), Constants.REVIEW);
        });

        binding.llSchedule.setOnClickListener(view -> {
            binding.switchStatus.setVisibility(View.GONE);
            binding.drawerLayout.closeDrawer(Gravity.START);
            binding.imgFilter.setVisibility(View.INVISIBLE);
            changeFragment(new ScheduleFragment(), Constants.SCHEDULE);
        });
        ...
        binding.llMenu.setOnClickListener(view -> {
            ...
            binding.drawerLayout.closeDrawer(Gravity.START);
            binding.imgFilter.setVisibility(View.INVISIBLE);
            changeFragment(new MenuFragment(), Constants.MENU);
        });
        ...
        binding.llLogOut.setOnClickListener(view -> {
            binding.drawerLayout.closeDrawer(Gravity.START);
            new AlertDialog.Builder(activity)
                    .setTitle(Constants.LOGOUT.substring(0, 1).toUpperCase() + Constants.LOGOUT.substring(1).toLowerCase())
                    .setMessage(Constants.LOGOUT_MSG)
                    .setPositiveButton(Constants.YES_LABEL, (dialog, which) -> {
                        logout();
                    })
                    .setNegativeButton(Constants.NO_LABEL, null)
    ...
    protected void onResume() {
        ...
        binding.tvMenu.setText(Constants.MENU.substring(0, 1).toUpperCase() + Constants.MENU.substring(1).toLowerCase());
        binding.tvReview.setText(Constants.REVIEW.substring(0, 1).toUpperCase() + Constants.REVIEW.substring(1).toLowerCase());
        binding.tvLogOue.setText(Constants.LOGOUT.substring(0, 1).toUpperCase() + Constants.LOGOUT.substring(1).toLowerCase());
        binding.tvSetting.setText(Constants.SETTING.substring(0, 1).toUpperCase() + Constants.SETTING.substring(1).toLowerCase());
        binding.tvScanCode.setText(Constants.SCANNER.substring(0, 1).toUpperCase() + Constants.SCANNER.substring(1).toLowerCase());
        binding.titleTv.setText(Constants.DASHBOARD.substring(0, 1).toUpperCase() + Constants.DASHBOARD.substring(1).toLowerCase());
        binding.tvSchedule.setText(Constants.SCHEDULE.substring(0, 1).toUpperCase() + Constants.SCHEDULE.substring(1).toLowerCase());
        binding.tvDashboard.setText(Constants.DASHBOARD.substring(0, 1).toUpperCase() + Constants.DASHBOARD.substring(1).toLowerCase());
        binding.tvOrderHistory.setText(Constants.ORDER_HISTORY.substring(0, 1).toUpperCase() + Constants.ORDER_HISTORY.substring(1).toLowerCase());
    }

    public void changeFragment(Fragment fragment, String title) {
        String header = title.substring(0, 1).toUpperCase() + title.substring(1).toLowerCase();
        binding.titleTv.setText(header);
    ...
    //TODO :Change restaurant status open or close
    private void changeRestaurantStatus(int status) {

        Utils.showProgress(activity);
        RetrofitHelper retrofitHelper = new RetrofitHelper();
        Call<ResponseBody> call;

        call = retrofitHelper.api().restaurantStatus(
        ...
        retrofitHelper.callApi(activity, call, new RetrofitHelper.ConnectionCallBack() {
            ...
            public void onSuccess(Response<ResponseBody> body) {
                ...
                try {
                    ...
                    Log.i("changeStatus", "onSuccess: " + response);
                    ...
                    if (jsonObject.getInt("status") == 0) {
                        ...
                        storeUserData.setInt(Constants.res_status, status);
                    }
    ...
    private void getProfile() {
        ...
        call = retrofitHelper.api().getProfile(
                storeUserData.getString(Constants.res_id),
                storeUserData.getString(Constants.token),
        ...
        retrofitHelper.callApi(activity, call, new RetrofitHelper.ConnectionCallBack() {
            ...
            public void onSuccess(Response<ResponseBody> body) {
                try {
                    ...
                    String response = body.body().string();
                    Log.i("TAG", "GET_PROFILE" + response);
                    ...
                    LoginPojo pojo = gson.fromJson(reader, LoginPojo.class);

                    if (pojo.status == 0) {
                        Utils.dismissProgress();
                    } else {

                        storeUserData.setString(Constants.res_id, pojo.responsedata.res_id);
                        storeUserData.setString(Constants.name, pojo.responsedata.name);
                        storeUserData.setString(Constants.email, pojo.responsedata.email);
                        storeUserData.setString(Constants.password, pojo.responsedata.password);
                        storeUserData.setString(Constants.building_no, pojo.responsedata.building_no);
                        storeUserData.setString(Constants.street, pojo.responsedata.street);
                        storeUserData.setString(Constants.province, pojo.responsedata.province);
                        storeUserData.setString(Constants.city, pojo.responsedata.city);
                        storeUserData.setString(Constants.country, pojo.responsedata.country);
                        storeUserData.setString(Constants.zip_code, pojo.responsedata.zip_code);
                        storeUserData.setString(Constants.latitude, pojo.responsedata.latitude);
                        storeUserData.setString(Constants.longitude, pojo.responsedata.longitude);
                        storeUserData.setString(Constants.website, pojo.responsedata.website);
                        storeUserData.setInt(Constants.res_status, pojo.responsedata.res_status);
                        storeUserData.setString(Constants.image, pojo.responsedata.image);
                        storeUserData.setInt(Constants.push_notification, pojo.responsedata.push_notification);
                        storeUserData.setInt(Constants.email_notification, pojo.responsedata.email_notification);
                        storeUserData.setString(Constants.token, pojo.responsedata.token);
                        storeUserData.setString(Constants.LANG_ID, pojo.responsedata.app_language);
                        storeUserData.setString( Constants.hour , ""+pojo.responsedata.avg_meal_hour );
                        storeUserData.setString( Constants.minute ,""+pojo.responsedata.avg_meal_min );
                        storeUserData.setString( Constants.capacity , pojo.responsedata.no_of_capacity );
    ...
    public void onBackPressed() {
        if (newDashboardFragment != null) {
            ...
            if (newDashboardFragment.isAdded() && newDashboardFragment.isVisible()) {
                super.onBackPressed();
            } else {
                binding.switchStatus.setVisibility(View.VISIBLE);
                binding.titleTv.setText(Constants.DASHBOARD);
                changeFragment(newDashboardFragment, Constants.DASHBOARD);
            }
